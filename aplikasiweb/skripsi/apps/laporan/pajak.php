...
<div class="print-hide" style="border-bottom:1px solid #ddd;overflow:hidden;margin:15px -20px;padding:0 20px;">
	...
	<form method="get" action="" style="float:left">
		<input type="hidden" name="sub" value="pajak.php" />
		<table cellpadding="5">
			<tr>
				<td>
				...
				<td>
					...
					<select name="tahun">
						...
						<?php
						$sql = "SELECT DISTINCT(YEAR(berlaku_pajak)) AS year FROM pajak ORDER BY berlaku_pajak ASC";
						$query = query($sql);
						while($data = mysqli_fetch_array($query)){
...
?>
<h2 class="content-title"><span>Laporan Masa Berlaku pajak<?php echo ($title ? '<br/>'.$title : '');?></span></h2>
...
if($_GET['tahun']){
	$whereClause[] = "YEAR(s.berlaku_pajak)='{$_GET['tahun']}'";
}
...
if($totalRows == 0){
	...
	while($data = mysqli_fetch_array($query)){
		...
		<tr class="<?php echo $data['status'];?>">
			...
			<td><?php echo date("d",strtotime($data['berlaku_pajak'])) .' '.monthToId(date('m', strtotime($data['berlaku_pajak']))). ' '.date('Y', strtotime($data['berlaku_pajak']));?></td>   
			...
			<td align="center"  class="print-hide"><a class="btn" href="?sub=pajak_detail.php&id=<?php echo $data['id_fpk'];?>">Detail</a></td>   
		</tr>
		<?php
		++$no;
	}
